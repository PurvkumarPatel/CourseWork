...
int main()
{
    ...
    while (!kbhit())
    {
        ...
        if (scoreRed >= 10 || scoreGreen >= 10 || scoreBlue >= 10)
        {
            clearviewport();
            setcolor(WHITE);
            settextstyle(6, 0, 4);
            const char *winners = "Congratulations";
            outtextxy(100, getmaxy() / 4, const_cast<char *>(winners));
            delay(500);
            if (scoreRed >= 10)
            {
                const char *red_w = "RED is WINNER";
                outtextxy(100, getmaxy() / 2 + 30, const_cast<char *>(red_w));
            }
            if (scoreGreen >= 10)
            {
                const char *green_w = "GREEN is WINNER";
                outtextxy(100, getmaxy() / 2 + 30, const_cast<char *>(green_w));
            }
            if (scoreBlue >= 10)
            {
                const char *blue_w = "BLUE is WINNER";
                outtextxy(100, getmaxy() / 2 + 30, const_cast<char *>(blue_w));
            }
            const char *out = "Press any key to EXIT";
            outtextxy(100, getmaxy() / 2 + 200, const_cast<char *>(out));
            break;
        }
