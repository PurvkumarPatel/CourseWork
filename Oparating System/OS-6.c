...
printf ("-->Enter size of physical address---> ");
...
printf ("Enter frame for %d page number(-1 if frame doesn't exist):) ",
	       i);
...
printf
	    ("Page no:-|> %d \nPage Offset:-|> %d\nFrame no:-|> %d\nPhysical address:-|> %d\n",
	     page_no, offset, pageTable[page_no].frame,
	     pageTable[page_no].frame * size_of_page + offset);
